    ...
    public sealed partial class AssetSurveyRepository
    {
        ...
        public DataTable Detail(long id, SurveyAccessContext access)
        {
            const string sql=@"SELECT s.*,d.DepartmentName,cat.CategoryName,con.ConditionName,m.UomCode,b.BuildingName,fl.FloorName,rm.RoomName,
              cu.DisplayName Custodian,cr.DisplayName CreatedByName FROM fa.AssetSurveys s JOIN mst.Departments d ON d.DepartmentId=s.DepartmentId
        ...
        public string SaveAttachment(long id,string type,string original,string stored,string relative,string contentType,long size,SurveyAccessContext access)
        {
            ...
            const string sql=@"DECLARE @Old nvarchar(500);SELECT @Old=RelativePath FROM fa.AssetSurveyAttachments WHERE SurveyId=@Id AND AttachmentType=@Type;
              ...
              WHEN MATCHED THEN UPDATE SET OriginalFileName=@Original,StoredFileName=@Stored,RelativePath=@Relative,ContentType=@ContentType,FileSizeBytes=@Size,UploadedByUserId=@UserId,UploadedUtc=SYSUTCDATETIME()
              WHEN NOT MATCHED THEN INSERT(SurveyId,AttachmentType,OriginalFileName,StoredFileName,RelativePath,ContentType,FileSizeBytes,UploadedByUserId) VALUES(@Id,@Type,@Original,@Stored,@Relative,@ContentType,@Size,@UserId);SELECT @Old;";
            using(var c=Db.OpenConnection())using(var cmd=new SqlCommand(sql,c)){cmd.Parameters.Add(Db.Parameter("@Id",id,SqlDbType.BigInt));cmd.Parameters.Add(Db.Parameter("@Type",type,SqlDbType.NVarChar,50));cmd.Parameters.Add(Db.Parameter("@Original",original,SqlDbType.NVarChar,260));cmd.Parameters.Add(Db.Parameter("@Stored",stored,SqlDbType.NVarChar,260));cmd.Parameters.Add(Db.Parameter("@Relative",relative,SqlDbType.NVarChar,500));cmd.Parameters.Add(Db.Parameter("@ContentType",contentType,SqlDbType.NVarChar,100));cmd.Parameters.Add(Db.Parameter("@Size",size,SqlDbType.BigInt));cmd.Parameters.Add(Db.Parameter("@UserId",access.UserId,SqlDbType.Int));return Convert.ToString(cmd.ExecuteScalar());}
        }
